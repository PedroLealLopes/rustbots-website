import React, {FC} from 'react'

export const Partners: FC = () => {
  return (
    <div className="Partners">
        <div className="content content mb-8 lg:mb-40">
            <h1>PARTNERS</h1>
            <p> Lorem ipsum dolor sit amet consectetur adipisicing elit. Natus laboriosam fugit error obcaecati dicta officiis, fugiat est pariatur corrupti vitae laborum voluptas vel perferendis, rem tempore incidunt aut, autem culpa!
                Lorem, ipsum dolor sit amet consectetur adipisicing elit. Minima praesentium ab mollitia earum repellat doloremque eum! Amet possimus dolore aut animi perferendis impedit, quaerat nemo temporibus cum tenetur aspernatur quo.
            </p>
        </div>
        <div className="partner-image">
          IMAGE PLACEHOLDER
        </div>
    </div>
  )
}